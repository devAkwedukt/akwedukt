...
export async function transformToPost(
  ...
  const title = decode(wpDoc.title?.rendered ?? "").trim();
  if (title) {
    doc.title = title;
  }
  ...
  if (typeof wpDoc.featured_media === "number" && wpDoc.featured_media > 0) {
    ...
    const cachedAssetId = existingImages[wpDoc.featured_media];
    if (cachedAssetId) {
      doc.featuredMedia = sanityIdToImageReference(cachedAssetId);
    } else {
